...
export class SolicitarTurnoComponent implements OnInit {
  ...
  async seleccionarEspecialidad(especialidad: string) {
    ...
    try {
      ...
      this.especialistas = await this.dbService.obtenerEspecialistasPorEspecialidad(especialidad);
    } catch (error) {
...
async seleccionarEspecialista(especialista: Especialista) {
  ...
  async seleccionarFecha(fecha: string) {
    ...
    try {
      ...
      this.horariosDisponibles = await this.dbService.obtenerHorariosDisponibles(
  ...
  if (this.esAdministrador) {
    ...
    try {
      const pacienteCompleto = await this.dbService.obtenerUsuarioPorEmail(this.usuarioActual.email);
      if (!pacienteCompleto) {
  ...
  async confirmarTurno() {
    ...
    if (this.esAdministrador && !this.pacienteSeleccionado) {
      this.mensajeError = 'Debe seleccionar un paciente antes de confirmar el turno';
      return;
    }
    ...
    try {
      ...
      if (this.esAdministrador) {
        ...
        const pacienteActual = await this.dbService.obtenerUsuarioPorEmail(this.usuarioActual.email);
      ...
      this.mensajeError = 'Error al crear el turno. Por favor intente nuevamente.';
    } finally {
  ...
  obtenerImagenEspecialidad(especialidad: string): string {
    return this.imagenesEspecialidades[especialidad.toLowerCase()] || 'assets/especialidades/default.png';
  }
...
async debugHorariosEspecialista(especialista: Especialista) {
  ...
  try {
    const todosLosHorarios = await this.dbService.obtenerTodosLosHorarios(especialista.id!);
    console.log('Todos los horarios del especialista:', todosLosHorarios);
    
    // Filtrar por especialidad seleccionada
    const horariosPorEspecialidad = todosLosHorarios.filter(h => h.especialidad === this.especialidadSeleccionada);
    console.log('Horarios para la especialidad seleccionada:', horariosPorEspecialidad);
